...
Adapted by François Vincent on march 06, 2013 for a better support of schemas
as required by Web2py
...
HELP = """
USAGE: extract_pgsql_models db host port user passwd [schema (default=public)]
...
def define_field(conn, table, field, pks):
    ...
    if SHOW_COMMENTS and comment is not None:
...
def define_table(conn, table):
    ...
    if SCHEMA is 'public':
        ...
        print "db.define_table('%s.%s'," % (SCHEMA, table)
    for field in fields:
